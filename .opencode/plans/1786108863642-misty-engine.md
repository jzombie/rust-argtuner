...
6. `Commands::Watch` (main.rs:43-54): make `--project` a **required** arg; remove `--db`:
   ```rust
   Watch {
       /// Path to the project directory (watches <dir>/trials.sqlite)
       #[arg(long, value_name = "PROJECT_DIR", required = true)]
       project: PathBuf,
       /// Polling interval (ms)
   ...
   ```
   (Note: `required = true` on the field is the clap-idiomatic way for an option that must be present;
   a positional `project: PathBuf` is the alternative — prefer keeping `--project <dir>` for clarity.)
...
8. **Tests**: add a small `tests/cli_watch.rs` (or extend an existing `src/main.rs` test) asserting
   clap rejects `watch` without `--project` and accepts `watch --project <dir>`. Use
   `Commands::command()` + `try_get_matches_from`. (There are currently no CLI tests — this is new.)
...
- `/Volumes/2TB Storage Vault/rust-argtuner/src/main.rs` — `--project` required, drop `--db`.
- `/Volumes/2TB Storage Vault/rust-argtuner/tests/cli_watch.rs` (new) — CLI arg tests.
