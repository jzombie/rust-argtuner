...
- **Protocol lines no longer echo to the terminal:** `RunnerOptions.suppress_protocol_echo` (enabled for trial runs) keeps `::ARGTUNER::` lines — including ones glued onto `\r` progress-bar fragments — out of the parent terminal while leaving the accumulated stdout (and post-exit parsing) intact. Human-readable fragments glued before a protocol payload are still echoed so progress bars keep animating.
- **Charts draw live step curves:** the Charts pane now renders per-step rows for the selected trial (falling back to epoch rows), fed live from the step publisher and the SQLite poll with replace-on-poll accumulation (no duplicates, no unbounded growth). The x-axis field is chosen by data properties — numeric, varying, preferring monotonic counters with deterministic tie-breaks — with no name lists anywhere (axis labels derive from the field name). The choice is sticky per trial: once picked it is kept while it still spreads the rows, so a per-epoch step counter resetting at an epoch boundary can no longer flip the axis mid-run (which plotted loss-vs-loss and read as "loss increasing").
