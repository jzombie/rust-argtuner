...
/// Re-export of the `#[tuner_args]` attribute macro so consumers only need
/// `argtuner-sdk`.
pub use argtuner_derive::tuner_args;
...
pub mod prelude {
    ...
    pub use crate::{
        tuner_args, EventKind, IpcChannel, MetricsBuilder, ParamHint, ParamKind, ParamRole,
        Params,
    };
}
...
/// The contract `#[tuner_args]` implements for a parameter struct: a plain
...
/// Unified entry point for a struct annotated with `#[tuner_args]`.
///
...
///    `(IpcChannel, T)`.
pub fn init<T: Params>() -> (IpcChannel, T) {
    init_with_args::<T>()
}

pub fn init_with_args<T: Params>() -> (IpcChannel, T) {
