...
pub struct TunerParam {
    ...
    /// Default value rendered as a string (also the clap default). `Borrowed`
    /// for literal/`&str`-const defaults (zero allocation); `Owned` for
    /// numeric/bool const expressions stringified at runtime (managed memory,
    /// never leaked).
    ...
    for p in T::tuner_params() {
        match p.role {
            ...
            ParamRole::Fixed => match p.kind {
                ParamKind::Bool => {
                    if p.default.as_deref() == Some("true") {
