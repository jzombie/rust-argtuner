...
```bash
cargo run -p argtuner -- run crates/tooling/loss-pattern-generator/loss-tuning-project
```

Viewing in TUI app:

```bash
cargo run -- watch --project crates/tooling/loss-pattern-generator/loss-tuning-project
```

## How it works

The project uses the `argtuner-loss-pattern-generator` crate (located in `../`) as the trial command.
