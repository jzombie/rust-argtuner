...
/// (e.g. `use argtuner_talkback::serde::Serialize;`) without adding serde as a
/// direct dependency.
pub use serde;
/// `clap` re-exported so the derive-generated code can reference it through
/// `argtuner_talkback::clap`, keeping clap off the consumer's dependency list.
#[cfg(feature = "clap")]
pub use clap;
...
#[cfg(feature = "clap")]
fn space_entry_toml(p: &ParamSpec) -> String {
    match p.kind {
        ParamKind::Float => {
            let mut out = format!(
                "[[space.params]]\ntype = \"Float\"\nname = \"{}\"\nmin = {}\nmax = {}\n",
